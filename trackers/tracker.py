...
class Tracker:
    ...
    def interpolate_ball_positions(self, ball_positions):
        ball_positions = [x.get(1, {}).get('bbox', []) for x in ball_positions]  #getting the track_id from 1 to whenever and then we will get bbox and if the track_id dictionary is empty then return empty list
        df_ball_positions = pd.DataFrame(ball_positions, columns=['x1', 'y1', 'x2', 'y2'])

        #Interepolate the missing values
    ...
    def draw_team_ball_control(self, frame, frame_num, team_ball_control):
        ...
        overlay = frame.copy()
        cv2.rectangle(overlay, (1350, 850), (1990, 970), (255,255,255), -1)
        alpha = 0.4
        cv2.addWeighted(overlay, alpha, frame, 1 - alpha, 0, frame)

        team_ball_control_till_frame = team_ball_control[ : frame_num + 1]

        #Get the number of the team that had the ball
    ...
    def draw_annotations(self, video_frames, tracks, team_ball_control):
        ...
        for frame_num, frame in enumerate(video_frames):
            frame = frame.copy()      #To not pollute the frames
